...
export async function generateCarePlanQuestions(symptoms: string): Promise<string[]> {
  if (!import.meta.env.VITE_OPENAI_API_KEY) {
    throw new Error('OpenAI API key is not configured');
  }
  ...
  try {
    const completion = await openai.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: 'You are a medical professional. Generate 5 relevant follow-up questions to gather more information about the patient\'s symptoms. Questions should be specific and help in creating a comprehensive care plan.'
        },
        {
          role: 'user',
...
export async function generateCarePlan(symptoms: string, answers: Record<string, string>): Promise<string> {
  if (!import.meta.env.VITE_OPENAI_API_KEY) {
    throw new Error('OpenAI API key is not configured');
  }
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: 'You are a medical professional. Create a comprehensive care plan based on the patient\'s symptoms and their answers to follow-up questions. Include lifestyle recommendations, self-care tips, and when to seek professional medical attention.'
        },
        {
          role: 'user',
    ...
    return completion.choices[0]?.message?.content || 'Unable to generate care plan';
  } catch (error) {
    console.error('OpenAI API Error:', error);
  ...
  try {
    const completion = await openai.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: 'You are a health and wellness expert. Provide a concise, practical daily health tip focusing on one key aspect of healthy living. Include a brief explanation of why it\'s important.'
        },
        {
          role: 'user',
